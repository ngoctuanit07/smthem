...
class Sm_ListingExtend_Block_List extends Mage_Catalog_Block_Product_Abstract
{
    ...
    public function _getCfg($attr = null)
    {
        ...
        $def_cfgs = Mage::getConfig()
            ...
            ->getNode('default/listingextend_cfg')->asArray();
        ...
        $_configs = (array)Mage::getStoreConfig("listingextend_cfg");
        if (empty($_configs)) return;
        $cfgs = array();

        foreach ($groups as $group) {
            $_cfgs = Mage::getStoreConfig('listingextend_cfg/' . $group . '');
    ...
    {
        if (!$this->_getConfig('active', 1)) return;
        $template_file = "sm/listingextend/default.phtml";
                    ...
                    if (!empty($_products)) {
						if( $listing_type == 2 ){
							...
							foreach ($_products as $_product) {
								...
								$under_price = (float)$this->_getConfig('under_price');
								$price = (float)$_product->getFinalPrice();
								if( $price && ($price <= $under_price) ){								
									$product_obj->id = $_product->getId();									
									$product_obj->title = $_product->getName();
									$image = $helper->getProductImage($_product, $this->_getConfig());
									$_image = $helper->_resizeImage($image, $image_config);
									$product_obj->_image = $_image;
									$product_obj->_description = $helper->_cleanText($_product->getDescription());
									$product_obj->_description = $helper->_trimEncode($_product->_description != '') ? $product_obj->_description : $helper->_cleanText($_product->getShortDescription());
									$product_obj->_description = $helper->_trimEncode($_product->_description != '') ? $helper->truncate($_product->_description, $this->_getConfig('product_description_maxlength')) : '';
									$product_obj->link = $_product->getProductUrl();
