...
class Sm_Listingdeals_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function _getCfg($attr = null)
	{
		...
		$defaults = array();
		$def_cfgs = Mage::getConfig()->loadModulesConfiguration('config.xml')->getNode('default/listingdeals_cfg')->asArray();
		...
		$_configs = (array)Mage::getStoreConfig("listingdeals_cfg");
		if (empty($_configs)) return;
		$cfgs = array();
		foreach ($groups as $group) {
			$_cfgs = Mage::getStoreConfig('listingdeals_cfg/' . $group . '');
	...
	{
		return md5(serialize(array('sm_listingdeals', $this->_config)));
	}
